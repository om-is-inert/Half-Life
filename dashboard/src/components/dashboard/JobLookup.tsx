...
export const JobLookup: React.FC<JobLookupProps> = ({ onSubmit, isLoading }) => {
  ...
  return (
    ...
    >
      {/* Status pill */}
      <div
        className="inline-flex items-center gap-2 px-3 py-1 rounded-full glass-card"
        style={{
          fontSize: '0.72rem',
      ...
      >
        ...
        >
          <div className="relative w-full h-full flex items-center">
            <div
              className="absolute left-4 top-1/2 -translate-y-1/2 flex items-center justify-center pointer-events-none transition-colors duration-200"
              style={{
                color: focused ? 'var(--color-text-1)' : 'var(--color-text-3)',
              }}
            >
            ...
            <input
              ...
              spellCheck={false}
              autoComplete="off"
              className="w-full h-full bg-transparent outline-none pl-11 pr-4"
              style={{
                fontFamily: 'var(--font-mono)',
                fontSize: '0.85rem',
      ...
      {/* Helper hint */}
      <p style={{ fontSize: '0.72rem', fontFamily: 'var(--font-mono)', color: 'var(--color-text-3)' }}>
        Run{' '}
        <span style={{ color: 'var(--color-text-2)' }}>python main.py</span>{' '}
        to get a Job ID
      </p>
    </motion.section>
  );
